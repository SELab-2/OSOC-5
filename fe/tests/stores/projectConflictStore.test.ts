...
describe('Skill Store', () => {
    ...
    it('resolveConflict', async () => {
        ...
        const projectConflictStore = useProjectConflictStore()
        await projectConflictStore.getConflictingProjects()
        let project = projectConflictStore.conflicts[0]
        let errortest = project.projects[0].suggestedStudents[0]
        errortest.student = new Student((await instance.get<StudentInterface>('students/1/')).data)
        project.projects[0].suggestedStudents = []
        // Should not pass since there are no suggested students
        await expect(async () => await projectConflictStore.resolveConflict(project.projects[0], project)).rejects.toThrow(/unexpected/)
        ...
        await expect(async () => await projectConflictStore.resolveConflict(project.projects[0], project)).rejects.toThrow(/delete/)
