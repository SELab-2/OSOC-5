...
} from '../../src/models/ProjectSuggestion'
...
describe('Project Store', () => {
  ...
  it('fetchSuggestedStudents', async () => {
    ...
    const data = (await instance.get<TempProjectSuggestion>('projects_sug/1'))
      .data
    const result = await projectStore.fetchSuggestedStudents([data])
  ...
  it('removeSuggestion', async () => {
    const projectStore = useProjectStore()
    const data1 = (
      await instance.get<ProjectSuggestionInterface>('projects_sug/1')
    ).data
    const data2 = (await instance.get<Project>('projects/1')).data
  ...
  it('getSkill', async () => {
    const projectStore = useProjectStore()
    const data = (await instance.get<TempProjectSkill>('proj_skills/1')).data
    const result = await projectStore.getSkill(data)
  ...
  it('getProject', async () => {
    const projectStore = useProjectStore()
    const result = await projectStore.getProject(1)
    expect(result).toBeInstanceOf(Project)
  })

  it('getOrFetchProject', async () => {
    const projectStore = useProjectStore()
    const result = await projectStore.getOrFetchProject('projects/2/')
  ...
  it('receiveSuggestion', async () => {
    const projectStore = useProjectStore()
    await projectStore.loadProjects()
    const data = (await instance.get<UserInterface>('coaches/1')).data
    await projectStore.receiveSuggestion(
      {
        project_id: '1',
        reason: 'test',
        coach: data,
  ...
  it('removeReceivedSuggestion', async () => {
    const projectStore = useProjectStore()
    await projectStore.loadProjects()
    await projectStore.removeReceivedSuggestion(
      { project_id: '1', student: 'students/1/', skill: 'skills/2' },
      'true'
    )
  })
  ...
  it('addProject', async () => {
    const projectStore = useProjectStore()
    const data = (await instance.get<Project>('projects/1')).data
    await projectStore.addProject(data)
    const result = expect(postcall_i).toBeCalledTimes(1)
  ...
  it('csv', async () => {
    const projectStore = useProjectStore()
    const result = await projectStore.csv()
